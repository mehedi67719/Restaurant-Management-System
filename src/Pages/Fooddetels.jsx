...
const Fooddetels = () => {
  ...
  if (!food) return <p className="text-center mt-20 text-gray-600 dark:text-gray-300 text-lg">Food not found.</p>;
      ...
      <div className="max-w-[95%] mx-auto">
        ...
        <div className="flex flex-col lg:flex-row gap-10 bg-white dark:bg-gray-900 rounded-3xl shadow-xl overflow-hidden">
          ...
          <div className="p-8 flex-1 flex flex-col justify-between">
            <div>
              <h2 className="text-3xl font-bold text-gray-900 dark:text-white mb-2">{food.name}</h2>
              <p className="text-sm text-gray-500 dark:text-gray-400 mb-4">{food.category}</p>
              <p className="text-primary font-semibold text-2xl mb-4">${food.price}</p>
              ...
              <p className="text-gray-700 dark:text-gray-300 mb-6">{food.description}</p>
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
              <span className={`font-semibold ${food.availability === "Available" ? "text-green-600" : "text-red-600"}`}>
                {food.availability}
              </span>
              <span className="font-medium text-gray-700 dark:text-gray-300">Prep Time: {food.prepTime}</span>
              <button className="btn-primary w-full sm:w-auto py-3 px-8 rounded-2xl text-lg mt-3 sm:mt-0">
                Add to Cart
              </button>
            </div>
          </div>
        ...
        {food.reviews && food.reviews.length > 0 && (
          <div className="mt-12 bg-white dark:bg-gray-900 rounded-3xl shadow-xl p-6">
            <h3 className="text-2xl font-bold mb-4 text-gray-900 dark:text-white">Customer Reviews</h3>
            <div className="flex flex-col gap-4">
              {food.reviews.map((rev, idx) => (
                <div key={idx} className="border-b border-gray-200 dark:border-gray-700 pb-3">
                  <div className="flex items-center justify-between">
                    <h4 className="font-semibold text-gray-900 dark:text-white">{rev.name}</h4>
